...
/**
 * The solver class which contains the main method to solve the triangle and give the user options
 * Contains methods to prompt for fields of the triangle.
 * Contains error checking methods to check if same letter was input more than once.
 * @author Clement Hoang and Pavan Jakhu
 */
public class Solver {
  /**
	 * Rounds the parameter to one decimal place and returns it
	 * @param double x
	...
	/**
	 * Prompts and gets a side length through user input (with error checking)
	 * @return the side length that the user inputs
	 * @author Clement Hoang and Pavan Jakhu
	 */
	...
	/**
	 * Prompts and gets an angle through user input (with error checking)
	 * @return the angle that the user inputs
	 * @author Clement Hoang and Pavan Jakhu
	 */
	...
	/**
	 * Prompts the user to select an option for the triangle.
	 * You can select to display specifications, perimeter or area of the triangle. You can also reset the specifications of the triangle or exit the program.
	...
	/**
	 * Prompts for a letter to enter an angle or side length for
	 * @return the letter that the user enters
	 * @author Clement Hoang and Pavan Jakhu
	 */
	...
	/**
	 * Checks if the letter entered has already been entered in the program already.
	 * @param String x, String lettersUsed
	 * @return true or false depending on if the letter has already been entered.
	 * @author Clement Hoang and Pavan Jakhu
	 */
	...
	/**
	 * Main method:
	 * calls methods for prompting for the letter and prompts for value of selected side length of angle,
	 * constructs the triangle,
	 * calls method to select an option to display the triangle's specifications, area and perimeter
	 * @param args
	 * @author Clement Hoang and Pavan Jakhu
	 */
	public static void main(String[] args) {
		...
		Triangle t; //declares the Triangle t, which will be constructed later in the program
		do {
			do { //this block of code resets all the variables for when the program restarts
				...
				do {
					...
					if (Triangle.solutions(a1, b1, c1, A1, B1, C1) == 0)  //checks to see if inputed values of the triangle construct a valid triangle
						System.out.println("A triangle cannot be constructed based on these specifications.\n\nRestarting program.\n");
			} while (Triangle.solutions(a1, b1, c1, A1, B1, C1) == 0); //loops back to beginning if a triangle cannot be constructed 
			t = new Triangle (A1, B1, C1, a1, b1, c1); //Constructs triangle
			do {
				menu = inputMenu(); //prompts and gets the option that the user selects
				if (menu == 1) { //display specifications is selected
					//outputs triangle specifications
					if (Triangle.solutions(a1, b1, c1, A1, B1, C1) == 2) //different outputs depending on whether there is 1 or 2 solutions
					...
					if (Triangle.solutions(a1, b1, c1, A1, B1, C1) == 2) { //checks to see if two triangles can be created, if so the program outputs the second triangle's specifications neatly 
						System.out.println("\nSolution 2:");
					...
					if (Triangle.solutions(a1, b1, c1, A1, B1, C1) == 2) { //displays second perimeter if applicable
					...
					if (Triangle.solutions(a1, b1, c1, A1, B1, C1) == 2) { //displays the second area if applicable
						System.out.println("Solution 2:");
